public class Compf extends Stack {
    ...
    private int symType(char c) {
        switch (c) {
            case '(': n=0;return SYM_LEFT;
            case ')': n=0;return SYM_RIGHT;
            case '+': case '-': case '*': case '/':case '|':case '&': n=0;return SYM_OPER;
            default: n++; return symOther(c);
        }
    }
    ...
    private void processSymbol(char c) {
        switch (symType(c)) {
            ...
            case SYM_RIGHT: processSuspendedSymbols(c);
            pop();
            break;

            case SYM_OPER: processSuspendedSymbols(c);
            push(c);
            break;
